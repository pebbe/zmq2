...
func TestPoller(t *testing.T) {
	...
	if rc, err := sb.Send(content, zmq.NOBLOCK); err != nil {
		t.Error("sb.Send NOBLOCK:", err)
	} else if rc != 32 {
		t.Error("sb.Send NOBLOCK:", err32)
	}
	...
	//  Receive message
	if msg, err := sc.Recv(zmq.NOBLOCK); err != nil {
		t.Error("sb.Recv NOBLOCK:", err)
	} else if msg != content {
